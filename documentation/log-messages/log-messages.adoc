...
Verify if the provided `memory-min-mb` and `memory-max-mb` parameters are correct and match amount of memory available in the operating system.
...
IMPORTANT: Some memory structures are allocated adhoc and not from the pool defined with `memory-min-mb` and `memory-max-mb` parameters.
...
TIP: If you don't have earlier schema but wish to continue having no information about column names and types, try with schemaless mode (`'flags': 2`).
...
The module which builds output stream is out of memory. Stream building is suspended until memory is released. If possible, configure higher memory limits.
...
After starting of replication and collecting schema file, OpenLogReplicator monitors all tables in the database which match defined schema and user mask.
...
If such event happens, replication can't start, and the following message is printed.
...
==== code 50066: "missing <table> table, <operation> failed"
...
Client requested a database which is not configured in the program.
