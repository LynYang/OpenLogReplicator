    ...
    void ReplicatorOnline::readSystemDictionariesMetadata(Schema* schema, typeScn targetScn) {
        ...
        try {
            ...
            for (auto ttSetIt: schema->xdbTtSetMapRowId) {
                SchemaXml* schemaXml = new SchemaXml(ctx, ttSetIt.second->tokSuf);
                schema->schemaXmlMap.insert_or_assign(ttSetIt.second->tokSuf, schemaXml);
                ...
                while (xdbXNmRet) {
                    schema->dictXdbXNmAdd(schemaXml, xdbXNmRowid, xdbNmNmSpcUri, xdbNmId);
                ...
                while (xdbXPtRet) {
                    schema->dictXdbXPtAdd(schemaXml, xdbXPtRowid, xdbPtPath, xdbPtId);
                ...
                while (xdbXQnRet) {
                    schema->dictXdbXQnAdd(schemaXml, xdbXQnRowid, xdbXQnNmSpcId, xdbXQnLocalName, xdbXQnFlags, xdbXQnId);
